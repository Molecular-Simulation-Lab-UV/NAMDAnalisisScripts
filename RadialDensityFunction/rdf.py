...
parser.add_argument('-f', '--first_frame', type=int, required=False, default=0, help='First frame for RDF calculation.')
parser.add_argument('-l', '--last_frame', type=int, required=False, default=0, help='Last frame for RDF calculation.')

arg = parser.parse_args()
inFile = open(arg.in_file, 'r')

first_frame = arg.first_frame if arg.first_frame != 0 else None
last_frame = arg.last_frame if arg.last_frame != 0 else None
dcdName = []
mainSel = None
selName = None
outName = 'outFile.out'
...
rdf_run.run(start=first_frame, stop=last_frame)
