...
for {set fr 0} {$fr < $numFrame} {incr fr} {
  ...
  foreach z [$wat get z] oldLab $oldList segname $segList resid $ridList {
    if {$z > $upperEnd} {
	  ...
	  #lappend list2 "$segname:$resid permeated through the nanotubes along +z direction at frame $fr"
        if {$fr >= $skipFrame} {
          incr num1
        }
      }
  ...
  lappend list2 "$fr	$num1	$num2	$num3"
}

############################ Abrir el archivo en modo de escritura
set fp [open "perm_clivoh27gggggggggggggggggggggggggg070.txt" "w"]

# Recorrer la lista2 y escribir cada fila en el archivo
puts $fp "fr	+z	-z	Total"
foreach row $list2 {
    puts $fp [join $row "\t"]
}
